    ...
    foreach ($this->options['backend'] as $id => $opts):
      ...
      switch ($id) {
        ...
        case 'warehouseOrder':
          ...
          if(empty($settings['backend']['username']) || empty($settings['backend']['password']) || empty($settings['backend']['appid']) || empty($settings['backend']['url'])){
            $this->messagesInformation("Incomplete configurations.<br>Please fill Username, Password, Application ID and Backend URL");
          } 
    ?>
